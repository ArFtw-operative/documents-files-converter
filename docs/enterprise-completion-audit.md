...
|---|---|---|---|
| 1 | Select and genuinely edit existing PDF text | Session-stable scene objects expose character spans and paragraph blocks with bounds, font resources, size/style, and editability. One-click inline paragraph edits remove underlying operators, reconstruct searchable text with regional reflow, prefer the detected font family, and validate the result. Type 3/protected runs are explicitly refused. | implemented |
...
| 11 | Searchable and editable OCR | Whole-document searchable OCR exists when binaries are installed, and capabilities/UI are limited to Tesseract languages actually present. Region OCR, confidence review, and editable reconstruction remain absent. | partial |
...
| 14 | Automated tests for every major tool | 45 tests cover command/session history, scene objects, range and paragraph text editing, font fallback, link create/update/delete, image/vector reconstruction, page import/geometry, the supported annotation matrix, AcroForm field/data/flatten flow, secure redaction, comparison, bookmarks/attachments, live preview, session reconciliation, validation, extraction/conversion, security, and authenticated export. Future enterprise studios still require their own tests. | partial |
...
| 17 | No primary placeholders | Every control exposed in the current PDF Studio maps to a real scene, command, preview, or export action. The non-functional Layers placeholder was removed; capabilities suppress unavailable OCR/font choices. | implemented |
...
| Native text engine | partial | Character mapping, range editing, paragraph reconstruction/reflow, glyph checks, font policy, baseline/style preservation, and one-click inline editing exist; complex shaping and collision UI remain. |
| Font subsystem | partial | Installed font discovery, embedded/subset-family matching, same-family fallback, glyph checks, explicit preserve/substitute/cancel policy, Type 3 protection, an internal font directory, and Noto/Liberation/DejaVu container coverage exist; embedding-right enforcement and advanced script shaping remain. |
...
| OCR Studio | partial | Local OCR adapters and installed-language capability discovery exist; region OCR, confidence model/review, and searchable-editable reconstruction remain absent. |
...
| Attachments/bookmarks/layers/navigation | partial | Native hierarchical bookmark add/update/subtree-delete and embedded attachment add/replace/delete/list/authenticated download are recoverable commands with Studio UI and tests. Link creation, rectangle/target update, and deletion are implemented. Page labels, destinations beyond page jumps, and editable optional-content layers remain. |
